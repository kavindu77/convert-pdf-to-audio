      ...
      <main className="relative z-10 max-w-4xl mx-auto px-6 py-12 space-y-8 w-full">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-slate-900">Form Data Extractor</h1>
          </div>
        ...
        <section className="rounded-2xl border border-slate-200 p-6 space-y-6 bg-white shadow-sm border border-slate-200/80 backdrop-blur-md">
          ...
          {!file ? (
            <div
              ...
              className="border-2 border-dashed border-slate-200 hover:border-amber-500/50 hover:bg-amber-500/[0.02] rounded-xl p-8 text-center cursor-pointer transition-all"
            >
              <input
                ref={inputRef}
                type="file"
              ...
              <p className="text-sm text-slate-900 font-medium">Click or drag interactive form PDF here</p>
            ...
            <div className="p-4 bg-slate-50 border border-slate-200 border border-slate-200 rounded-xl flex items-center justify-between">
              ...
              <button
                ...
                className="text-xs text-slate-400 hover:text-slate-900 transition-colors animate-fade-in"
              >
                Clear
              </button>
            </div>
          ...
          {file && !isProcessing && fields.length === 0 && (
            <div className="flex justify-end">
              <button
                ...
                className="px-6 py-2.5 bg-amber-600 hover:bg-amber-700 text-slate-900 rounded-xl text-sm font-semibold transition-all hover:scale-[1.02]"
              >
                Extract Data Fields
              </button>
            </div>
          ...
          {isProcessing && (
            <div className="space-y-3 p-4 bg-slate-50 border border-slate-200 border border-slate-200 rounded-xl flex flex-col items-center">
              ...
              <p className="text-xs text-slate-500 font-medium">Reading form data tree...</p>
            </div>
          )}
        ...
        {fields.length > 0 && (
          <section className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-semibold text-slate-900 text-lg">Extracted Form Data ({fields.length} fields)</h2>
              {csvUrl && (
                <a
                  href={csvUrl}
                  download={file ? `${file.name.replace(".pdf", "")}_form_data.csv` : "form_data.csv"}
                  className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-slate-900 rounded-xl text-xs font-semibold transition-all hover:scale-[1.02]"
                >
            ...
            <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm border border-slate-200/80 backdrop-blur-md">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="border-b border-slate-200 bg-slate-50 border border-slate-200">
                      <th className="p-3 text-slate-500 font-semibold uppercase tracking-wider">Field Name</th>
                      <th className="p-3 text-slate-500 font-semibold uppercase tracking-wider">Field Type</th>
                      <th className="p-3 text-slate-500 font-semibold uppercase tracking-wider">Filled Value</th>
                    </tr>
                  </thead>
                  <tbody>
                    {fields.map((f, idx) => (
                      <tr key={idx} className="border-b border-slate-200/60 hover:bg-white shadow-sm border border-slate-200/80 transition-colors">
                        <td className="p-3 font-semibold text-slate-900 font-mono">{f.name}</td>
                        <td className="p-3 text-slate-500">{f.type}</td>
