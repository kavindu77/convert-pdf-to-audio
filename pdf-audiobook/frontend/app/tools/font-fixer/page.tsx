      ...
      <main className="relative z-10 max-w-4xl mx-auto px-6 py-12 space-y-8 w-full">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-slate-900">Font Problem Fixer</h1>
          </div>
        ...
        <section className="rounded-2xl border border-slate-200 p-6 space-y-6 bg-white shadow-sm border border-slate-200/80 backdrop-blur-md">
          ...
          {!file ? (
            <div
              ...
              className="border-2 border-dashed border-slate-200 hover:border-rose-500/50 hover:bg-rose-500/[0.02] rounded-xl p-8 text-center cursor-pointer transition-all"
            >
              <input
                ref={inputRef}
                type="file"
          ...
          {file && !isProcessing && !report && (
            <div className="flex justify-end">
              <button
                onClick={runAnalysis}
                className="px-6 py-2.5 bg-rose-600 hover:bg-rose-700 text-slate-900 rounded-xl text-sm font-semibold transition-all hover:scale-[1.02]"
              >
        ...
        {report && (
          <section className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="font-semibold text-slate-900 text-lg font-medium">Font Health Summary</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 bg-white shadow-sm border border-slate-200/80 border border-slate-200/60 rounded-xl flex items-center justify-between">
                <div>
                  <p className="text-xs text-slate-400">Unembedded Fonts</p>
                  <p className={`text-xl font-bold ${report.unembeddedCount > 0 ? "text-yellow-400" : "text-green-700"}`}>
                    {report.unembeddedCount}
                  </p>
                </div>
                {report.unembeddedCount > 0 ? <AlertTriangle className="text-yellow-400" size={24} /> : <CheckCircle2 className="text-green-700" size={24} />}
              </div>
              <div className="p-4 bg-white shadow-sm border border-slate-200/80 border border-slate-200/60 rounded-xl flex items-center justify-between">
                <div>
                  <p className="text-xs text-slate-400">Corrupted Text Maps</p>
                  <p className={`text-xl font-bold ${report.brokenMapCount > 0 ? "text-red-800" : "text-green-700"}`}>
                    {report.brokenMapCount}
                  </p>
                </div>
                {report.brokenMapCount > 0 ? <XCircle className="text-red-800" size={24} /> : <CheckCircle2 className="text-green-700" size={24} />}
              </div>
            </div>

            {report.warnings.length > 0 && (
              <div className="p-4 bg-yellow-50 border border-yellow-250 text-yellow-800 text-xs rounded-xl space-y-1">
            ...
            <div className="p-5 bg-white shadow-sm border border-slate-200/80 border border-slate-200 rounded-2xl space-y-3">
              <h3 className="font-bold text-sm text-slate-900">Document Font Inventory</h3>
              
              {report.fonts.length === 0 ? (
                <p className="text-xs text-slate-400 italic">No custom fonts detected in this PDF (using default Helvetica).</p>
              ) : (
                <div className="space-y-2">
                  {report.fonts.map((f, idx) => (
                    <div key={idx} className="p-3.5 bg-slate-50 border border-slate-200 border border-slate-200 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                      <div>
                        <h4 className="font-bold text-slate-900 text-sm">{f.name}</h4>
                        <p className="text-slate-400">Type: {f.type} · Pages: {f.pagesUsed.join(", ")}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-0.5 rounded-full font-semibold text-[10px] border ${
                          f.isEmbedded
                            ? "bg-green-50 border-green-500/25 text-green-700"
                            : "bg-yellow-50 border-yellow-500/25 text-yellow-400"
                        }`}>
                        ...
                        <span className={`px-2 py-0.5 rounded-full font-semibold text-[10px] border ${
                          f.hasToUnicode
                            ? "bg-green-50 border-green-500/25 text-green-700"
                            : "bg-red-50 border-red-500/25 text-red-800"
                        }`}>
