      ...
      <main className="relative z-10 max-w-4xl mx-auto px-6 py-12 space-y-8 w-full">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-slate-900">Color Page Detector</h1>
          </div>
        ...
        {report && (
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="p-4 bg-white shadow-sm border border-slate-200/80 border border-slate-200/60 rounded-xl text-center space-y-1">
                <p className="text-xs text-slate-400">B&W Pages</p>
                <p className="text-2xl font-bold text-slate-900">{report.bwPagesCount}</p>
              </div>
              <div className="p-4 bg-green-500/5 border border-green-500/10 rounded-xl text-center space-y-1 flex flex-col items-center justify-center">
                <p className="text-xs text-green-700 flex items-center gap-0.5 justify-center"><DollarSign size={12} /> Printing Savings</p>
                <p className="text-xl font-extrabold text-green-800">${calculateSavings().toFixed(2)}</p>
              </div>
            </div>

            {/* Pages Grid */}
            <div className="p-5 bg-white shadow-sm border border-slate-200/80 border border-slate-200 rounded-2xl space-y-3">
              <h3 className="font-bold text-sm text-slate-900">Color Mapping Layout</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {report.pages.map((p) => (
                  <div
                    key={p.pageNumber}
                    className={`relative p-2 rounded-xl border-2 flex flex-col items-center gap-2 bg-gray-900 ${
                      p.type === "color" ? "border-emerald-500/60" : "border-slate-200"
                    }`}
                  >
                    <img
                      src={p.thumbnailUrl}
                    ...
                    <div className="flex items-center justify-between w-full text-[10px] pt-1">
                      <span className="text-slate-500 font-medium">Page {p.pageNumber}</span>
                      <span className={`px-1.5 py-0.5 rounded font-bold uppercase ${
                        p.type === "color" ? "bg-emerald-500/20 text-emerald-300" : "bg-slate-50 border border-slate-200 text-slate-400"
                      }`}>
                        {p.type}
                      </span>
                    </div>
