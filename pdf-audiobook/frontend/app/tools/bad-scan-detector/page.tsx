      ...
      <main className="relative z-10 max-w-4xl mx-auto px-6 py-12 space-y-8 w-full">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 rounded-xl bg-pink-50 border border-pink-200 flex items-center justify-center">
              <ScanLine size={20} className="text-pink-600" />
            </div>
            <h1 className="text-3xl font-bold text-slate-900">Bad Scan Detector</h1>
          </div>
        ...
        {report && (
          <section className="space-y-6">
            <h2 className="font-semibold text-slate-900 text-lg font-medium">Quality Audit Report</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-4 bg-white shadow-sm border border-slate-200/80 border border-slate-200/60 rounded-xl space-y-1">
                <p className="text-xs text-slate-400">Blank Pages detected</p>
                <p className={`text-xl font-bold ${report.blankPages.length > 0 ? "text-yellow-400" : "text-green-700"}`}>
                  {report.blankPages.length}
                </p>
                {report.blankPages.length > 0 && <p className="text-[10px] text-slate-500">Pages: {report.blankPages.join(", ")}</p>}
              </div>
              <div className="p-4 bg-white shadow-sm border border-slate-200/80 border border-slate-200/60 rounded-xl space-y-1">
                <p className="text-xs text-slate-400">Blurry Pages detected</p>
                <p className={`text-xl font-bold ${report.blurryPages.length > 0 ? "text-red-800" : "text-green-700"}`}>
                  {report.blurryPages.length}
                </p>
                {report.blurryPages.length > 0 && <p className="text-[10px] text-slate-500">Pages: {report.blurryPages.join(", ")}</p>}
              </div>
              <div className="p-4 bg-white shadow-sm border border-slate-200/80 border border-slate-200/60 rounded-xl space-y-1">
                <p className="text-xs text-slate-400">Low Contrast Pages</p>
                <p className={`text-xl font-bold ${report.lowContrastPages.length > 0 ? "text-yellow-400" : "text-green-700"}`}>
                  {report.lowContrastPages.length}
                </p>
                {report.lowContrastPages.length > 0 && <p className="text-[10px] text-slate-500">Pages: {report.lowContrastPages.join(", ")}</p>}
              </div>
            </div>
            ...
            <div className="p-5 bg-white shadow-sm border border-slate-200/80 border border-slate-200 rounded-2xl space-y-3">
              <h3 className="font-bold text-sm text-slate-900">Full Page Scan Log</h3>
              ...
              {report.warningsCount === 0 ? (
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                  {report.pages.map((p) => {
                    ...
                    return (
                      <div key={p.pageNumber} className="p-3 bg-slate-50 border border-slate-200 border border-slate-200 rounded-xl flex items-center justify-between text-xs">
                        <span className="font-semibold text-slate-900">Page {p.pageNumber}</span>
                        <div className="flex gap-2">
                          {p.isBlank && <span className="bg-yellow-500/20 text-yellow-800 px-2 py-0.5 rounded border border-yellow-500/10">Blank</span>}
                          ...
                          {p.isLowContrast && <span className="bg-yellow-500/20 text-yellow-800 px-2 py-0.5 rounded border border-yellow-500/10">Low Contrast</span>}
                        </div>
                      </div>
                    );
                  })}
