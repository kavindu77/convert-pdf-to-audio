      ...
      <main className="relative z-10 max-w-4xl mx-auto px-6 py-12 space-y-8 w-full">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-slate-900">Attachment Inspector</h1>
          </div>
        ...
        <section className="rounded-2xl border border-slate-200 p-6 space-y-6 bg-white shadow-sm border border-slate-200/80 backdrop-blur-md">
          <h2 className="font-semibold text-slate-600 flex items-center gap-2">
            <span className="w-6 h-6 rounded-full bg-indigo-500/20 border border-indigo-500/40 text-indigo-600 text-xs flex items-center justify-center">1</span>
            Upload PDF
          </h2>
          ...
          {!file ? (
            <div
              ...
              className="border-2 border-dashed border-slate-200 hover:border-indigo-500/50 hover:bg-indigo-500/[0.02] rounded-xl p-8 text-center cursor-pointer transition-all"
            >
              <input
                ref={inputRef}
                type="file"
            ...
            <div className="p-4 bg-slate-50 border border-slate-200 border border-slate-200 rounded-xl flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-indigo-500/10 rounded-lg flex items-center justify-center border border-indigo-200 text-indigo-600">
                  <FileText size={20} />
                </div>
                <div>
          ...
          {file && !isProcessing && attachments.length === 0 && (
            <div className="flex justify-end">
              <button
                ...
                className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-slate-900 rounded-xl text-sm font-semibold transition-all hover:scale-[1.02]"
              >
                Inspect Attachments
              </button>
            </div>
            ...
            {attachments.length > 0 ? (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="font-semibold text-slate-900 text-lg">Detected Embedded Attachments ({attachments.length})</h2>
                  {!sanitizedUrl && (
                    <button
                      onClick={removeAttachments}
                      className="flex items-center gap-1.5 px-4 py-2 bg-red-500/15 border border-red-200 text-red-800 hover:bg-red-500/20 rounded-xl text-xs font-semibold transition-colors"
                    >
                ...
                <div className="space-y-2">
                  {attachments.map((att, idx) => (
                    <div key={idx} className="p-4 bg-white shadow-sm border border-slate-200/80 border border-slate-200 rounded-xl flex items-center justify-between">
                      <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 px-3 py-1.5 bg-slate-50 border border-slate-200 hover:bg-slate-100 border border-slate-200 rounded-lg text-xs font-medium transition-colors"
                      >
